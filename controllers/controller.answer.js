            ...
            bodyData.map(async (data, k) => {
                ...
                if (fileName == undefined || fileName === false) {
                    ...
                    data.fileName.map((v1, k1) => {
                        try {
                            answerService.addAnswer(other, optionChoiceId, userId, questionId, survey_headers_id, building_id, answeredDate, keyValue, countryId, subQuestionId, surveySectionId, req.body.id[k] + "_" + v1);
                            count++;
                            if (count == targetCount) 
                                resolve({answeredCount: count});
